...
def add_message():
    if request.method == "OPTIONS": # CORS preflight
        ...
        # ^root\[\'fields\'\]\[\'(\d+)\'\]\[\'properties\'\]\[\'([a-z\-?\d?A-Z09]+)\'\]$
        ...
        for arr_add in arr_items_added:
            ...
            if (field_match):
                ...
                if (field_rules_add_match):
                    ...
                    # Add a additional arr if the field does not have it
                    if ('additional' not in field_changes[field_index]):
                        field_changes[field_index]['additional'] = []
                    # No match we found in field setting, field properties, field rules - return to the client the full path
                    field_changes[field_index]['additional'].append({ 'propertyName': path, 'value': arr_add.t2, 'action': 'ADDED' })
            ...
            if (field_match):
                ...
                if (field_rules_add_match):
                    ...
                    if ('additional' not in field_changes[field_index]):
                        field_changes[field_index]['additional'] = []
                    # No match we found in field setting, field properties, field rules - return to the client the full path
                    field_changes[field_index]['additional'].append({ 'propertyName': path, 'value': arr_remove.t1, 'action': 'REMOVED' })
        ...
        for remove in dic_items_removed:
            ...
            if (field_match):
                ...
                if (field_setting_changed_match or field_properties_changed_match):
                    ...
                    # No match we found in field setting, field properties, field rules - return to the client the full path
                    field_changes[field_index]['removedProperties'].append({ 'propertyName': path })
        ...
        for add in dic_items_added:
            ...
            if (field_match):
                ...
                if (field_setting_changed_match or field_properties_changed_match):
                    ...
                    # No match we found in field setting, field properties, field rules - return to the client the full path
                    field_changes[field_index]['addedProperties'].append({ 'propertyName': path })
        ...
        for change in dic_items_changed:
            ...
            if (field_match):
                ...
                if (field_setting_changed_match or field_properties_changed_match):
                    ...
                    # No match we found in field setting, field properties, field rules - return to the client the full path
                    field_changes[field_index]['changedProperties'].append({ 'propertyName': path, 'oldValue': change.t1, 'newValue': change.t2 })
